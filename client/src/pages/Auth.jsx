...
export default function Auth() {
  ...
  const [form, setForm] = useState({ name: '', email: 'demo@focusflow.app', password: 'FocusFlow123!' });
      ...
      <section className="relative mx-auto grid min-h-screen w-full max-w-7xl items-center gap-10 px-5 py-10 lg:grid-cols-[1.05fr_.95fr] lg:px-8">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
          ...
          <div className="mt-8 grid max-w-2xl gap-3 sm:grid-cols-3">
            {['XP loops', 'Streak shields', 'Analytics pulse'].map((item) => (
              <div key={item} className="rounded-3xl border border-white/12 bg-white/10 p-4 backdrop-blur-xl">
                <p className="font-extrabold">{item}</p>
                <p className="mt-2 text-sm text-white/54">Built for repeat action.</p>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        >
          <div className="mb-6 flex rounded-2xl bg-white/8 p-1">
            {['login', 'signup'].map((item) => (
              <button
                ...
                className={`flex-1 rounded-xl px-4 py-3 text-sm font-extrabold capitalize transition ${mode === item ? 'bg-white text-slate-950' : 'text-white/58'}`}
              >
                {item}
              </button>
            ))}
          ...
          <div className="space-y-4">
            {mode === 'signup' && <Input label="Name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Your name" />}
